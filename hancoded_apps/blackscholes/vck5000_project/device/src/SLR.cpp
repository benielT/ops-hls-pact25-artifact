...
void process_SLR(hls::stream <t_pkt> &in, hls::stream <t_pkt> &a_in, hls::stream <t_pkt> &b_in, hls::stream <t_pkt> &c_in,
		hls::stream <t_pkt> &out, hls::stream <t_pkt> &a_out, hls::stream <t_pkt> &b_out, hls::stream <t_pkt> &c_out,
    ...
    static hls::stream<uint256_dt> streamArray_a[SLR0_P_STAGE + 1];
    static hls::stream<uint256_dt> streamArray_b[SLR0_P_STAGE + 1];
    static hls::stream<uint256_dt> streamArray_c[SLR0_P_STAGE + 1];
    #pragma HLS STREAM variable = streamArray depth = 10
	#pragma HLS STREAM variable = streamArray_a depth = 10
	#pragma HLS STREAM variable = streamArray_b depth = 10
	#pragma HLS STREAM variable = streamArray_c depth = 10
	...
	data_g.gridsize = (data_g.end_row* batches) * data_g.end_index;
...
//	float beta = computeParam.risk_free_rate * computeParam.delta_t;
	...
	axis2_fifo256(a_in, streamArray_a[0], data_g.total_itr_256);
	axis2_fifo256(b_in, streamArray_b[0], data_g.total_itr_256);
	axis2_fifo256(c_in, streamArray_c[0], data_g.total_itr_256);

	// unrolling iterative loop
	for(int i = 0; i < SLR0_P_STAGE; i++){
		#pragma HLS unroll
		process_grid(streamArray[i], streamArray_a[i], streamArray_b[i], streamArray_c[i],
				streamArray[i+1], streamArray_a[i+1], streamArray_b[i+1], streamArray_c[i+1], data_g);
	}
	...
	fifo256_2axis(streamArray_a[SLR0_P_STAGE], a_out, data_g.total_itr_256);
	fifo256_2axis(streamArray_b[SLR0_P_STAGE], b_out, data_g.total_itr_256);
	fifo256_2axis(streamArray_c[SLR0_P_STAGE], c_out, data_g.total_itr_256);
}
...
extern "C" {
	void stencil_SLR(
			...
			hls::stream <t_pkt> &in,
			hls::stream <t_pkt> &a_in,
			hls::stream <t_pkt> &b_in,
			hls::stream <t_pkt> &c_in,
			hls::stream <t_pkt> &out,
			hls::stream <t_pkt> &a_out,
			hls::stream <t_pkt> &b_out,
			hls::stream <t_pkt> &c_out)
	{
			...
			#pragma HLS INTERFACE axis port = a_in  register
			#pragma HLS INTERFACE axis port = b_in  register
			#pragma HLS INTERFACE axis port = c_in  register
			#pragma HLS INTERFACE axis port = out register
			#pragma HLS INTERFACE axis port = a_out register
			#pragma HLS INTERFACE axis port = b_out register
			#pragma HLS INTERFACE axis port = c_out register
			...
			for(int i =  0; i < count*2; i++){
				process_SLR(in, a_in, b_in, c_in, out, a_out, b_out, c_out, xdim0, size0, size1, batches, computeParam);
			}
