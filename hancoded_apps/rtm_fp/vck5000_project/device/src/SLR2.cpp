...
#include "derives_calc_ytep_k1.h"
#include "derives_calc_ytep_k2.h"
#include "derives_calc_ytep_k3.h"
#include "derives_calc_ytep_k4.h"
...
//		printf("%d \n", itr);
		...
		t_pkt tmp = in.read();
#ifdef DEBUG_LOG
        printf("[HLS_DEBUG_LOG] SLR2| %s | reading from axis iter: %d\n",__func__, itr);
#endif
		out << tmp.data;
	}
}
...
static void fifo256_2axis(hls::stream <uint256_dt> &in, hls::stream<t_pkt> &out, const int gridsize_da){
	for (int itr = 0; itr < gridsize_da; itr++){
		...
		tmp.data = in.read();
#ifdef DEBUG_LOG
        printf("[HLS_DEBUG_LOG] SLR2| %s | writing to axis iter: %d\n",__func__, itr);
#endif
		out.write(tmp);
	}
}

void process_rtm_SLR2 (hls::stream <t_pkt> &in, hls::stream <t_pkt> &out,
...
void kernel_SLR2(
	...
	for(int i =  0; i < 2*count; i++){
		...
		process_rtm_SLR2(in, out, sizex, sizey, sizez, xdim_aligned, batch);
	}
