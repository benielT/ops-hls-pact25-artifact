...
// #ifdef DEBUG_LOG
//         printf("[HLS_DEBUG_LOG]| %s | reading row iter: %d\n",__func__, itr);
// #endif
		rd_buffer << arg0[itr];
	}
}
...
static void axis2_fifo256(hls::stream <t_pkt> &in, hls::stream<uint256_dt> &out, const int gridsize_da){
	for (int itr = 0; itr < gridsize_da; itr++){
		...
		t_pkt tmp = in.read();
#ifdef DEBUG_LOG
        printf("[HLS_DEBUG_LOG] read_write| %s | reading from axis iter: %d\n",__func__, itr);
#endif
		out << tmp.data;
	}
}
...
static void fifo256_2axis(hls::stream <uint256_dt> &in, hls::stream<t_pkt> &out, const int gridsize_da){
	for (int itr = 0; itr < gridsize_da; itr++){
		...
		tmp.data = in.read();
#ifdef DEBUG_LOG
        printf("[HLS_DEBUG_LOG] read_write| %s | writing to axis iter: %d\n",__func__, itr);
#endif
		out.write(tmp);
	}
}
