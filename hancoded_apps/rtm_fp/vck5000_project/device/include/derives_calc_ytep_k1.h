...
#ifdef DEBUG_LOG
        printf("[DEBUG][INTERNAL][k1] read values arg: (");
                for (int ri = 0; ri < 8; ri++)
                {
                    DataConv tmpConverter;
                    tmpConverter.i = s_4_4_8.range((ri + 1)*32 - 1, ri * 32);
...
#ifdef DEBUG_LOG
            printf("[DEBUG][INTERNAL][k1] writing values wr_buffer: (");
            for (int ri = 0; ri < 8; ri++)
                {
                    DataConv tmpConverter;
                    tmpConverter.i = update_j.range((ri + 1)*32 - 1, ri * 32);

                    printf("%f, ", tmpConverter.f);
                }
                printf(")\n, iter: ", itr);
            printf("[DEBUG][INTERNAL][k1] writing values yy: (");
            for (int ri = 0; ri < 8; ri++)
                {
                    DataConv tmpConverter;
                    tmpConverter.i = s_4_4_4.range((ri + 1)*32 - 1, ri * 32);

                    printf("%f, ", tmpConverter.f);
                }
                printf(")\n");
            printf("[DEBUG][INTERNAL][k1] writing values yy_final: (");
            for (int ri = 0; ri < 8; ri++)
                {
                    DataConv tmpConverter;
                    tmpConverter.i = yy_final_vec.range((ri + 1)*32 - 1, ri * 32);
