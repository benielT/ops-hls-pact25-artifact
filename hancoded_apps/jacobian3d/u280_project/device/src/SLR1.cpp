...
void process_SLR (hls::stream <t_pkt> &in1, hls::stream <t_pkt> &out1, hls::stream <t_pkt> &in2, hls::stream <t_pkt> &out2,
	...
	fifo256_2axis(streamArray[P_STAGE_SLR/2], out1, gridsize_da);
	axis2_fifo256(in2, streamArray[P_STAGE_SLR/2+1], gridsize_da);

    for(int i = P_STAGE_SLR/2 + 1; i < P_STAGE_SLR+1; i++){
		#pragma HLS unroll
    	process_tile( streamArray[i], streamArray[i+1], data_g);
    }

	fifo256_2axis(streamArray[P_STAGE_SLR+1], out2, gridsize_da);
