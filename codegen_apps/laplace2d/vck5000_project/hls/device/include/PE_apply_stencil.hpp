// Auto-generated at 2025-07-29 02:44:01.961904 by ops-translator

#pragma once
#include <ops_hls_kernel_support.h>
#include "common_config.hpp"

static constexpr unsigned short read_num_points_apply_stencil = 5;
static constexpr unsigned short read_stencil_size_apply_stencil = 3;
static constexpr unsigned short read_stencil_dim_apply_stencil = 2;

static constexpr unsigned short write_num_points_apply_stencil = 1;
static constexpr unsigned short write_stencil_size_apply_stencil = 1;
static constexpr unsigned short write_stencil_dim_apply_stencil = 2;

static constexpr unsigned short vector_factor_apply_stencil_0 = vector_factor * 1;
static constexpr unsigned short axis_data_width_l_apply_stencil_0 = vector_factor_apply_stencil_0 * data_width;
typedef ap_uint<axis_data_width_l_apply_stencil_0> widen_apply_stencil_0_dt;
static constexpr unsigned short vector_factor_apply_stencil_1 = vector_factor * 1;
static constexpr unsigned short axis_data_width_l_apply_stencil_1 = vector_factor_apply_stencil_1 * data_width;
typedef ap_uint<axis_data_width_l_apply_stencil_1> widen_apply_stencil_1_dt;

typedef ::hls::stream<widen_apply_stencil_0_dt> widen_stream_apply_stencil_0_dt;
typedef ::hls::stream<widen_apply_stencil_1_dt> widen_stream_apply_stencil_1_dt;

/*
    ArgDat(id=0, loc=/home/x_thileeb/repos/ops-hls-pact25-artifact/apps/laplace2d/vck5000_project/laplace2d.cpp/297:17, access_type=AccessType.OPS_READ, opt=True, dat_id=0, global_dat_id=0, stencil_id=S2D_5pt)
    ArgDat(id=1, loc=/home/x_thileeb/repos/ops-hls-pact25-artifact/apps/laplace2d/vck5000_project/laplace2d.cpp/298:17, access_type=AccessType.OPS_WRITE, opt=True, dat_id=1, global_dat_id=1, stencil_id=S2D_00)
*/
inline void kernel_apply_stencil_core(
...
{
#ifdef DEBUG_LOG
    printf("[KERNEL_INTERNAL_CORE]|%s| starting kernel core: kernel_apply_stencil_core\n",__func__);
#endif

    float tmp0 = reg_0_3 + reg_0_1;
    float tmp1 = reg_0_0 + reg_0_4;
    reg_1_0 =  0.25f * (tmp0 + tmp1);
...
#ifdef DEBUG_LOG
    printf("[KERNEL_INTERNAL_CORE]|%s| exiting: kernel_apply_stencil_core\n",__func__);
#endif
}

class Stencil_apply_stencil : public ops::hls::StencilCoreV2<stencil_type, read_num_points_apply_stencil, vector_factor, ops::hls::CoefTypes::CONST_COEF,
        read_stencil_size_apply_stencil, read_stencil_dim_apply_stencil>
{
    using ops::hls::StencilCoreV2<stencil_type, read_num_points_apply_stencil, vector_factor, ops::hls::CoefTypes::CONST_COEF,
            read_stencil_size_apply_stencil, read_stencil_dim_apply_stencil>::m_stencilConfig;
public:

    void stencilRun(
            widen_stream_apply_stencil_0_dt& arg0_rd_buffer,
            widen_stream_apply_stencil_1_dt& arg1_wr_buffer
        )
    {
    ...
    //  *** stencil description and read & write point definitions  ****
        ...
        */
        ...
        unsigned short S2D_5pt_buf_r0_1_p0_rd = 0;
        unsigned short S2D_5pt_buf_r0_1_p0_wr = 1;
        // read point: (1,2,0), write point: (2,1,0)
        unsigned short S2D_5pt_buf_r1_2_p0_rd = 0;
        unsigned short S2D_5pt_buf_r1_2_p0_wr = 1;
    ...
    //  *** data read write boundary definitions ****
            unsigned int S2D_5pt_read_lb_itr = 0 * stencilConfig.grid_size[0];
            unsigned int S2D_5pt_read_ub_itr = (0 + stencilConfig.grid_size[1]) * stencilConfig.grid_size[0];
    ...
    */
    ...
    // arg0(d_A)
        widen_apply_stencil_0_dt arg0_read_val = 0;
    // arg1(d_Anew)
        widen_apply_stencil_1_dt arg1_update_val;
    //  *** widen stencil values holder & window buffers ****

        // arg0(d_A)
        widen_apply_stencil_0_dt arg0_widenStencilValues[read_num_points_apply_stencil];
        #pragma HLS ARRAY_PARTITION variable = arg0_widenStencilValues dim = 1 complete

        widen_apply_stencil_0_dt arg0_buf_r0_1_p0[max_depth];
        #pragma HLS BIND_STORAGE variable = arg0_buf_r0_1_p0 type = ram_s2p latency=2
        widen_apply_stencil_0_dt arg0_buf_r1_2_p0[max_depth];
        ...
        #pragma HLS PIPELINE II=1
            ...
            spc_temp_blocking_read:
            {
...
#ifdef DEBUG_LOG
                printf("[DEBUG][INTERNAL][apply_stencil_PE_%d] loop params before update i(%d), "\
                    "j(%d), "\
                    "S2D_5pt_buf_r0_1_p0_rd: %d, "\
                    "S2D_5pt_buf_r0_1_p0_wr: %d, "\
                    "S2D_5pt_buf_r1_2_p0_rd: %d, "\
                    "S2D_5pt_buf_r1_2_p0_wr: %d, "\
                    "reg_itr(%d)\n", m_PEId, i, 
                    j,
                    S2D_5pt_buf_r0_1_p0_rd, 
                    S2D_5pt_buf_r0_1_p0_wr, 
                    S2D_5pt_buf_r1_2_p0_rd, 
                    S2D_5pt_buf_r1_2_p0_wr, 
                    reg_itr);
#endif
                if (cond_x_terminate)
                    i = 0;
                ...
                if (cond_x_terminate && cond_y_terminate)
                    ...
                    j++;
                    bool S2D_5pt_read_cond = (reg_itr < S2D_5pt_read_ub_itr) and (reg_itr >= S2D_5pt_read_lb_itr);
                /*    bool read_cond  =  (reg_itr < read_ub_itr) and (reg_itr >= read_lb_itr); */

                    if (S2D_5pt_read_cond)
                    {
                ...
                arg0_widenStencilValues[0] = arg0_buf_r0_1_p0[S2D_5pt_buf_r0_1_p0_wr];                
                arg0_widenStencilValues[1] = arg0_widenStencilValues[2];                
                arg0_widenStencilValues[2] = arg0_widenStencilValues[3];                
                arg0_widenStencilValues[3] = arg0_buf_r1_2_p0[S2D_5pt_buf_r1_2_p0_wr];                
                arg0_buf_r0_1_p0[S2D_5pt_buf_r0_1_p0_rd] = arg0_widenStencilValues[1];                
                arg0_widenStencilValues[4] = arg0_read_val;                
                arg0_buf_r1_2_p0[S2D_5pt_buf_r1_2_p0_rd] = arg0_widenStencilValues[4];

                bool cond_end_of_line_buff_S2D_5pt_buf_r0_1_p0_rd = S2D_5pt_buf_r0_1_p0_rd >= (stencilConfig.grid_size[0] - 1);
                bool cond_end_of_line_buff_S2D_5pt_buf_r0_1_p0_wr = S2D_5pt_buf_r0_1_p0_wr >= (stencilConfig.grid_size[0] - 1);

                if (cond_end_of_line_buff_S2D_5pt_buf_r0_1_p0_rd)
                    S2D_5pt_buf_r0_1_p0_rd = 0;
                else
                    S2D_5pt_buf_r0_1_p0_rd++;

                if (cond_end_of_line_buff_S2D_5pt_buf_r0_1_p0_wr)
                    S2D_5pt_buf_r0_1_p0_wr = 0;
                else
                    S2D_5pt_buf_r0_1_p0_wr++;
                bool cond_end_of_line_buff_S2D_5pt_buf_r1_2_p0_rd = S2D_5pt_buf_r1_2_p0_rd >= (stencilConfig.grid_size[0] - 1);
                bool cond_end_of_line_buff_S2D_5pt_buf_r1_2_p0_wr = S2D_5pt_buf_r1_2_p0_wr >= (stencilConfig.grid_size[0] - 1);

                if (cond_end_of_line_buff_S2D_5pt_buf_r1_2_p0_rd)
                    S2D_5pt_buf_r1_2_p0_rd = 0;
                else
                    S2D_5pt_buf_r1_2_p0_rd++;

                if (cond_end_of_line_buff_S2D_5pt_buf_r1_2_p0_wr)
                    S2D_5pt_buf_r1_2_p0_wr = 0;
                else
                    S2D_5pt_buf_r1_2_p0_wr++;

#ifdef DEBUG_LOG
                printf("[DEBUG][INTERNAL][apply_stencil_PE_%d] loop params after update i(%d), "\
                                "j(%d), "\
                                "S2D_5pt_buf_r0_1_p0_rd(%d), "\
                                "S2D_5pt_buf_r0_1_p0_wr(%d), "\
                                "S2D_5pt_buf_r1_2_p0_rd(%d), "\
                ...
                printf("[DEBUG][INTERNAL][apply_stencil_PE_%d] --------------------------------------------------------\n\n", m_PEId);

                printf("[DEBUG][INTERNAL][apply_stencil_PE_%d] read values arg0: (", m_PEId);
...
#pragma HLS UNROLL factor=vector_factor
...
#ifdef DEBUG_LOG
                printf("[DEBUG][INTERNAL][apply_stencil_PE_%d] index=(%d, %d), lowerbound=(%d, %d), upperbound=(%d, %d), neg_cond=%d\n", m_PEId, index, j,
    ...
    // [1, 0]
            //dat_id: 1, dat: d_Anew, swap_id: 0, swap_dat: d_A
            ...
            write:
            {
                ...
                if (cond_write)
                {
                    ...
                    printf("[DEBUG][INTERNAL][apply_stencil_PE_%d] wirte values arg1: (", m_PEId);
                    for (int ri = 0; ri < vector_factor; ri++)
                    {
                        ops::hls::DataConv tmpConverter;
...
void kernel_apply_stencil_PE(const short& PEId, const ops::hls::StencilConfigCore& stencilConfig,
            //d_A
    widen_stream_apply_stencil_0_dt& arg0_rd_buffer,
            //d_Anew
    widen_stream_apply_stencil_1_dt& arg1_wr_buffer        
