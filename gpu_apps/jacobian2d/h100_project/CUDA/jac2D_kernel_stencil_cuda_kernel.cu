...
//
__constant__ int dims_jac2D_kernel_stencil [2][3];
static int dims_jac2D_kernel_stencil_h [2][3] = {0};

//user function


__global__ void ops_jac2D_kernel_stencil(
...
int bounds_2_l, int bounds_2_u) {
  ...
  if (n_0 < bounds_0_u && n_1 < bounds_1_u && n_2 < bounds_2_u) {
    const ACC<float> u(dims_jac2D_kernel_stencil[0][0], dims_jac2D_kernel_stencil[0][1], u_p + n_0 + n_1 * dims_jac2D_kernel_stencil[0][0] + n_2 * dims_jac2D_kernel_stencil[0][0] * dims_jac2D_kernel_stencil[0][1]);
    ACC<float> u2(dims_jac2D_kernel_stencil[1][0], dims_jac2D_kernel_stencil[1][1], u2_p + n_0 + n_1 * dims_jac2D_kernel_stencil[1][0] + n_2 * dims_jac2D_kernel_stencil[1][0] * dims_jac2D_kernel_stencil[1][1]);
...
// host stub function
#ifndef OPS_LAZY
void ops_par_loop_jac2D_kernel_stencil(char const *name, ops_block block, int dim, int* range,
...
#endif
#else
void ops_par_loop_jac2D_kernel_stencil_execute(const char *name, ops_block block, int blockidx_start, int blockidx_end, int dim, int *range, int nargs, ops_arg* args) {
  #ifdef OPS_BATCHED
  ...
  if (OPS_instance::getOPSInstance()->OPS_diags > 1) {
    ops_timing_realloc(3,"jac2D_kernel_stencil");
    OPS_instance::getOPSInstance()->OPS_kernels[3].count++;
    ops_timers_core(&__c2,&__t2);
  }

  #ifdef OPS_DEBUG
  ops_register_args(args, "jac2D_kernel_stencil");
  #endif
  ...
  #endif
  if (args[0].dat->size[0] != dims_jac2D_kernel_stencil_h[0][0] || args[0].dat->size[1] != dims_jac2D_kernel_stencil_h[0][1] || args[0].dat->size[2] != dims_jac2D_kernel_stencil_h[0][2] || args[1].dat->size[0] != dims_jac2D_kernel_stencil_h[1][0] || args[1].dat->size[1] != dims_jac2D_kernel_stencil_h[1][1] || args[1].dat->size[2] != dims_jac2D_kernel_stencil_h[1][2]) {
    dims_jac2D_kernel_stencil_h[0][0] = args[0].dat->size[0];
    dims_jac2D_kernel_stencil_h[0][1] = args[0].dat->size[1];
    dims_jac2D_kernel_stencil_h[0][2] = args[0].dat->size[2];
    dims_jac2D_kernel_stencil_h[1][0] = args[1].dat->size[0];
    dims_jac2D_kernel_stencil_h[1][1] = args[1].dat->size[1];
    dims_jac2D_kernel_stencil_h[1][2] = args[1].dat->size[2];
    cutilSafeCall(cudaMemcpyToSymbol( dims_jac2D_kernel_stencil, dims_jac2D_kernel_stencil_h, sizeof(dims_jac2D_kernel_stencil)));
  }
  ...
  if (x_size > 0 && y_size > 0)
    ops_jac2D_kernel_stencil<<<grid, tblock >>> (  u_p, u2_p,         bounds_0_l, bounds_0_u, bounds_1_l, bounds_1_u,
...
void ops_par_loop_jac2D_kernel_stencil(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0, ops_arg arg1) {
  ...
  desc->function = ops_par_loop_jac2D_kernel_stencil_execute;
  if (OPS_instance::getOPSInstance()->OPS_diags > 1) {
    ops_timing_realloc(3,"jac2D_kernel_stencil");
  }
