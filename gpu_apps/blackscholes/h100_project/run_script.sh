...
for params in "${parameter_sets[@]}"; do
    ...
    for i in $(seq 1 $batch); do
        ...
        if [[ "${CXXFLAGS}" == *"-DPOWER_PROFILE"* ]]; then
            ${POWER_PROFILE_SCRIPT} ${SCRIPT_DIR}/${APP}_cuda -sizex="${sizex}" -iters="${iters}" -batch="${bsize}" OPS_BLOCK_SIZE_X=64 OPS_BLOCK_SIZE_Y=16 OPS_BLOCK_SIZE_Z=1
            ...
            ${SCRIPT_DIR}/${APP}_cuda -sizex="${sizex}" -iters="${iters}" -batch="${bsize}" OPS_BLOCK_SIZE_X=64 OPS_BLOCK_SIZE_Y=16 OPS_BLOCK_SIZE_Z=1
