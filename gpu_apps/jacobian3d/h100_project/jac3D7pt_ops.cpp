...
void ops_par_loop_jac3D_kernel_stencil(char const *, ops_block, int , int*,
  ops_arg,
  ops_arg );
...
#endif
{
  ...
  ops_init_backend();
    ...
    for (unsigned int bat = 0; bat < batches; bat++)
    {
        ...
        ops_par_loop_kernel_copy("kernel_update", blocks[bat], 3, full_range,
                     ops_arg_dat(u[bat], 1, S2D_00, "float", OPS_READ),
                     ops_arg_dat(u2[bat], 1, S2D_00, "float", OPS_WRITE));
#else
        ops_par_loop_kernel_populate("kernel_populate", blocks[bat], 3, full_range,
                     ops_arg_dat(u[bat], 1, S3D_00, "float", OPS_WRITE));

        ops_par_loop_kernel_initialguess("kernel_initialguess", blocks[bat], 3, internal_range,
                     ops_arg_dat(u[bat], 1, S3D_00, "float", OPS_WRITE));
    ...
    for (unsigned int bat = 0; bat < batches; bat++)
    {
        ...
        {
            ops_par_loop_jac3D_kernel_stencil("jac2D_kernel_stencil", blocks[bat], 3, internal_range,
                         ops_arg_dat(u[bat], 1, S3D_7PT, "float", OPS_READ),
                         ops_arg_dat(u2[bat], 1, S3D_00, "float", OPS_WRITE));

            ops_par_loop_kernel_copy("kernel_update", blocks[bat], 3, internal_range,
