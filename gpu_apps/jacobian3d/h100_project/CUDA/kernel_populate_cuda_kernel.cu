...
//
__constant__ int dims_kernel_populate [1][4];
static int dims_kernel_populate_h [1][4] = {0};

//user function


__global__ void ops_kernel_populate(
...
int bounds_2_l, int bounds_2_u, int bounds_3_l, int bounds_3_u) {
  ...
  if (n_0 < bounds_0_u && n_1 < bounds_1_u && n_2 < bounds_2_u && n_3 < bounds_3_u) {
    ACC<float> u(dims_kernel_populate[0][0], dims_kernel_populate[0][1], dims_kernel_populate[0][2], u_p + n_0 + n_1 * dims_kernel_populate[0][0] + n_2 * dims_kernel_populate[0][0] * dims_kernel_populate[0][1] + n_3 * dims_kernel_populate[0][0] * dims_kernel_populate[0][1] * dims_kernel_populate[0][2]);
...
// host stub function
#ifndef OPS_LAZY
void ops_par_loop_kernel_populate(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0) {
...
#endif
#else
void ops_par_loop_kernel_populate_execute(const char *name, ops_block block, int blockidx_start, int blockidx_end, int dim, int *range, int nargs, ops_arg* args) {
  #ifdef OPS_BATCHED
  ...
  if (OPS_instance::getOPSInstance()->OPS_diags > 1) {
    ops_timing_realloc(1,"kernel_populate");
    OPS_instance::getOPSInstance()->OPS_kernels[1].count++;
    ops_timers_core(&__c2,&__t2);
  }

  #ifdef OPS_DEBUG
  ops_register_args(args, "kernel_populate");
  #endif
  ...
  #endif
  if (args[0].dat->size[0] != dims_kernel_populate_h[0][0] || args[0].dat->size[1] != dims_kernel_populate_h[0][1] || args[0].dat->size[2] != dims_kernel_populate_h[0][2] || args[0].dat->size[3] != dims_kernel_populate_h[0][3]) {
    dims_kernel_populate_h[0][0] = args[0].dat->size[0];
    dims_kernel_populate_h[0][1] = args[0].dat->size[1];
    dims_kernel_populate_h[0][2] = args[0].dat->size[2];
    dims_kernel_populate_h[0][2] = args[0].dat->size[3];
    cutilSafeCall(cudaMemcpyToSymbol( dims_kernel_populate, dims_kernel_populate_h, sizeof(dims_kernel_populate)));
  }
  ...
  if (x_size > 0 && y_size > 0 && z_size > 0)
    ops_kernel_populate<<<grid, tblock >>> (  u_p,         bounds_0_l, bounds_0_u, bounds_1_l, bounds_1_u,
...
void ops_par_loop_kernel_populate(char const *name, ops_block block, int dim, int* range,
 ops_arg arg0) {
  ...
  desc->function = ops_par_loop_kernel_populate_execute;
  if (OPS_instance::getOPSInstance()->OPS_diags > 1) {
    ops_timing_realloc(1,"kernel_populate");
  }
