...
if [[ "${CXXFLAGS}" == *"-DPOWER_PROFILE"* ]]; then
    ...
    parameter_sets=(
            "30,30,30,60000,1,1"
            "50,50,50,60000,1,1"
            "100,100,100,60000,1,1"
            "300,300,300,60000,1,1"
            "150,150,150,60000,1,1"
            "200,200,200,60000,1,1"
            "250,250,250,60000,1,1"
            "30,30,30,60000,10,10"
            "50,50,50,60000,10,10"
            "100,100,100,60000,10,10"
            "150,150,150,60000,10,10"
            "200,200,200,60000,10,10"
            "250,250,250,60000,10,10"
            "300,300,300,60000,10,10"
            "30,30,30,60000,50,50"
            "50,50,50,60000,50,50"
            "100,100,100,60000,50,50"
            "150,150,150,60000,50,50"
    ...
    parameter_sets=(
            # "30,30,30,60000,1,10"
            # "50,50,50,60000,1,10"
            # "100,100,100,60000,1,2"
            # "300,300,300,60000,1,2"
            # "150,150,150,60000,1,2"
            # "200,200,200,60000,1,2"
            # "250,250,250,60000,1,2"
            # "30,30,30,60000,10,20"
            # "50,50,50,60000,10,20"
            # "100,100,100,60000,10,20"
            # "150,150,150,60000,10,20"
            # "200,200,200,60000,10,20"
            # "250,250,250,60000,10,20"
            # "300,300,300,60000,10,20"
            "30,30,30,60000,50,50"
            "50,50,50,60000,50,50"
            "100,100,100,60000,50,50"
            "150,150,150,60000,50,50"
...
for params in "${parameter_sets[@]}"; do
    IFS=',' read -r sizex sizey sizez iters bsize batch <<< "$params"

    if [[ -z "$sizex" || -z "$sizey"  || -z "$sizez" || -z "$iters" || -z "$bsize" || -z "$batch" ]]; then
    ...
    echo "-----------------------------------------------------------------"
    echo "Running with sizex=${sizex}, sizey=${sizey}, sizez=${sizez}, iters=${iters}, batch_size=${bsize}, batch=${batch}"
    echo "-----------------------------------------------------------------"

    if [[ "${CXXFLAGS}" == *"-DPOWER_PROFILE"* ]]; then
        ${POWER_PROFILE_SCRIPT} ${SCRIPT_DIR}/${APP}_cuda -sizex="${sizex}" -sizey="${sizey}" -sizez="${sizez}" -iters="${iters}" -n_sys="${bsize}" -batch="${batch}" OPS_BLOCK_SIZE_X=10 OPS_BLOCK_SIZE_Y=10 OPS_BLOCK_SIZE_Z=10
        ...
        if [ -f "${POWER_PROFILE_FILE}" ]; then
            ...
            new_filename="${POWER_PROFILE_DIR}/${bsize}-batch/${sizex}_${sizey}_${sizez}_${POWER_PROFILE_FILE}"
        ...
        ${SCRIPT_DIR}/${APP}_cuda -sizex="${sizex}" -sizey="${sizey}" -sizez="${sizez}" -iters="${iters}" -n_sys="${bsize}" -batch="${batch}" OPS_BLOCK_SIZE_X=10 OPS_BLOCK_SIZE_Y=10 OPS_BLOCK_SIZE_Z=10
        ...
        if [ -f "${PROFILE_FILE}" ]; then
            # Construct the new filename for the profile directory
            new_filename="${PROFILE_DIR}/${bsize}-batch/${sizex}_${sizey}_${sizez}_${PROFILE_FILE}"
