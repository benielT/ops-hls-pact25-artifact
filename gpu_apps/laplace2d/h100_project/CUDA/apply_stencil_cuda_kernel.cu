...
//
__constant__ int dims_apply_stencil [2][3];
static int dims_apply_stencil_h [2][3] = {0};
...
int bounds_2_l, int bounds_2_u) {
  ...
  if (n_0 < bounds_0_u && n_1 < bounds_1_u && n_2 < bounds_2_u) {
    const ACC<float> A(dims_apply_stencil[0][0], dims_apply_stencil[0][1], A_p + n_0 + n_1 * dims_apply_stencil[0][0] + n_2 * dims_apply_stencil[0][0] * dims_apply_stencil[0][1]);
    ACC<float> Anew(dims_apply_stencil[1][0], dims_apply_stencil[1][1], Anew_p + n_0 + n_1 * dims_apply_stencil[1][0] + n_2 * dims_apply_stencil[1][0] * dims_apply_stencil[1][1]);
...
#endif
#else
void ops_par_loop_apply_stencil_execute(const char *name, ops_block block, int blockidx_start, int blockidx_end, int dim, int *range, int nargs, ops_arg* args) {
  #ifdef OPS_BATCHED
  ...
  #endif
  if (args[0].dat->size[0] != dims_apply_stencil_h[0][0] || args[0].dat->size[1] != dims_apply_stencil_h[0][1] || args[0].dat->size[2] != dims_apply_stencil_h[0][2] || args[1].dat->size[0] != dims_apply_stencil_h[1][0] || args[1].dat->size[1] != dims_apply_stencil_h[1][1] || args[1].dat->size[2] != dims_apply_stencil_h[1][2]) {
    dims_apply_stencil_h[0][0] = args[0].dat->size[0];
    dims_apply_stencil_h[0][1] = args[0].dat->size[1];
    dims_apply_stencil_h[0][2] = args[0].dat->size[2];
    dims_apply_stencil_h[1][0] = args[1].dat->size[0];
    dims_apply_stencil_h[1][1] = args[1].dat->size[1];
    dims_apply_stencil_h[1][2] = args[1].dat->size[2];
  ...
  float * __restrict__ Anew_p = (float *)(args[1].data_d + args[1].dat->base_offset + blockidx_start * args[1].dat->batch_offset);
  ...
  if (x_size > 0 && y_size > 0)
    ops_apply_stencil<<<grid, tblock >>> (  A_p, Anew_p,         bounds_0_l, bounds_0_u, bounds_1_l, bounds_1_u,
